...
function Update () {
	...
	if (on){
		if(Time.time > nextbulletTraceTime){
			...
			var bulletVelocity = newBulletTrace.transform.right * Random.Range(-1,1);
			bulletVelocity += newBulletTrace.transform.up * Random.Range(-1,1);
			//newBulletTrace.GetComponent("bulletTrace").velocity = 700.0f;
		}
	}
}